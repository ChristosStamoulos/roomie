...
public class Master {
    ...
    public static void init() {
        ...
        String filename = "app\\src\\main\\java\\com\\example\\roomie\\backend\\config\\master.config";
    ...
    private static void processRequest(int type, Chunk chunk){
        if(Arrays.asList(1,6,7,8).contains(type)) {
            // Iterate all workers
        ...
        } else if (Arrays.asList(2,3,5).contains(type)) {
            ...
            try {
                data1 = new JSONObject( (String) chunk.getData());
