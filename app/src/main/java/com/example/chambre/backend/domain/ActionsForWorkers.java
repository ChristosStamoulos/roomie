...
public class ActionsForWorkers extends Thread {
    ...
    private void processRequest(int type, Chunk chunk) {
        switch (type) {
            case 1:     //user's request, searches for rooms with given filters
                ArrayList<Room> filteredRooms = null;
                try {
                    filteredRooms = findRoomByFilter(new JSONObject( (String)data.getData()));
     ...
     * @return          an ArrayList of objects Room
     */
    private ArrayList<Room> findRoomByFilter(JSONObject filter) throws JSONException {
